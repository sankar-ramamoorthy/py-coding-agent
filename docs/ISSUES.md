...
|----|-------|--------|--------|--------|-------|
| ISS-003 | Skills/dynamic tools execute arbitrary code before approval | in-progress | fix-skill-tool-approval-gate | docs/project-audit-2026-08-02.md (C-03) | Fixing: `SkillRegistry.load()`/`reload_skill()` gated on a hash-ledger approval record (not just the in-file status field) before `exec_module` runs; `/approve` re-validates current `skill.py` via `validate_skill_py` before writing the ledger; `ENABLE_DYNAMIC_TOOLS` (default false) gates dynamic-tool auto-loading, plus static validation before `exec_module` and before `create_tool()` writes to disk. Live-confirmed pre-fix: a `status: proposed` skill's module code executes at load time regardless. See `specs/004-fix-skill-tool-approval-gate/`. |
