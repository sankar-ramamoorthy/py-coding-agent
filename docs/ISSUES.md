...
| ISS-007 | Add dual Ollama backend selection (local + remote GPU) with runtime model switching | in-progress | ollama-dual-backend | 2026-08-03 session, user request | Local laptop Ollama is slow; remote GPU desktop (1080 Ti) Ollama reachable over Tailscale at `http://100.105.24.12:11434`. Adds `ollama-remote`/`ollama-local`/`ollama-auto` `provider_registry.py` entries wrapping the existing `OllamaProvider` (new optional `base_url` param) plus a connectivity-probe fallback (`ollama-auto` prefers remote, falls back to local). Model switching already works via the existing `/provider <name> <model>` command — no command-dispatch changes needed. See `specs/002-ollama-dual-backend/`. |
