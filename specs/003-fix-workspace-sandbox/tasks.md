...
- [X] T001 Create `tests/utils/` directory
...
- [X] T002 Add `ENABLE_SHELL_TOOL` (truthy-string-parsed, default `False`) and
...
- [X] T003 [P] [US1] Write `tests/utils/test_path_utils.py`: valid in-workspace path still
...
- [X] T004 [US1] Fix `resolve_safe_path` in `py_mono/utils/path_utils.py`: replace
...
- [X] T005 [US1] Run `pytest tests/utils/test_path_utils.py -v` and confirm all US1 cases
      from T003 pass
...
- [X] T006 [US2] Add additional-allowed-paths test cases to
...
- [X] T007 [US2] No new production code expected — `T004`'s `allowed_roots` list already
      incorporates `ADDITIONAL_ALLOWED_PATHS` from `T002`'s config addition. This task is
      verification only.
- [X] T008 [US2] Run `pytest tests/utils/test_path_utils.py -v` and confirm all US2 cases
      from T006 pass
...
- [X] T009 [P] [US3] Write `tests/tools/test_shell.py`: `ENABLE_SHELL_TOOL` defaults false,
...
- [X] T010 [US3] Add `DEFAULT_SHELL_TIMEOUT_SECONDS = 30` to `py_mono/tools/shell.py`, pass
...
- [X] T011 [US3] Extract `build_base_tools(enable_shell: Optional[bool] = None) -> list` in
...
- [X] T012 [US3] Run `pytest tests/tools/test_shell.py -v` and confirm all US3 cases from
      T009 pass
...
- [X] T013 [US4] Update `docker-compose.yml`: `.:/app` → `.:/app:ro`; add
      `ENABLE_SHELL_TOOL=${ENABLE_SHELL_TOOL:-false}` and
      `ADDITIONAL_ALLOWED_PATHS=${ADDITIONAL_ALLOWED_PATHS:-}` to the `environment:` block
- [X] T014 [US4] Update `.env.example` with commented-out `ENABLE_SHELL_TOOL=true` and
...
- [X] T015 [P] Update `docs/adr/ADR-001-safe-execution-of-tools.md` in place: Status
...
- [X] T016 Run `python -m compileall -q py_mono` — repo-wide syntax gate
- [X] T017 Run `pytest` (full suite) at the repo root — confirm nothing existing regressed,
      the two pre-existing `ISS-005` failures remain exactly as documented
- [X] T018 Real, non-mocked verification inside the actual Docker container — all confirmed:
      audit's literal probe now raises `ValueError` (was silently accepted before); `shell`
      absent by default, present with `ENABLE_SHELL_TOOL=true`; `sleep 60` terminated at
      exactly 30.0s with the timeout message; `ls /` and `cat /etc/os-release` still succeed
      with shell enabled (reach genuinely unchanged); after rebuild with the new mount,
      writes to `/workspace`, `/app/dynamic_tools`, `/app/skills` all still succeed,
      `uv pip install --system` still succeeds (site-packages, unaffected), and a direct
      write to `/app/py_mono/...` now fails with `[Errno 30] Read-only file system`
